...
const ControlArea: FC<ControlProps> = forwardRef(({ className }, nodeRef: ForwardedRef<HTMLDivElement>) => {
  const tags = [
    {
      title: "布局",
      children: [
  ...
  return (
    <div className={`absolute left-0 top-0 z-20 h-full w-72 overflow-auto ${className}`} ref={nodeRef}>
      {tags.map((item, index) => (
        <section className={`rounded-lg bg-gray-1100 px-4 py-4 dark:bg-purple-1100 ${index ? "mt-8" : ""}`} key={item.title}>
          <div className="pb-3 text-base font-semibold text-gray-1200 dark:text-purple-1200">{item.title}</div>
